...
void onRender(CRules@ this)
{
	if (showTime > 0)
	{
		...
		string display = getTranslatedString("\n  Server: {SERVERNAME}\n").replace("{SERVERNAME}", servername);

		display += getTranslatedString("  Gamemode: {GAMEMODE}").replace("{GAMEMODE}", getTranslatedString(name));

		if (name != info && info != "")
			display += " \n\n"+getTranslatedString(" Objectives")+": " + getTranslatedString(info);
