...
{
	//CBlob@ connectedBlob = getBlobByNetworkID(this.get_u16("ConnectedID"));
	//if (connectedBlob !is null && !this.hasTag("Active"))
	//{
	//	this.SetLight(true);
	//	this.getSprite().SetFrameIndex(1);
	//	this.Tag("Active");
...
void onRender(CSprite@ this)
{
	...
	if (connectedBlob is null) return;

	f32 length = (connectedBlob.getPosition() - blob.getPosition()).getLength();
	GUI::DrawSpline(connectedBlob.getPosition(), blob.getPosition()+Vec2f(0,4), connectedBlob.getPosition()+Vec2f(length/3,length*0.1), blob.getPosition()+Vec2f((-length/3),length*0.1), 6, color_white);
}