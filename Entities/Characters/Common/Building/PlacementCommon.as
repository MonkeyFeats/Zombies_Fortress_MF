...
CBlob@ ElectricalBlobAtPos(CBlob@ this, Vec2f p, CBlob @blob)
{
	CMap@ map = this.getMap();	
	Vec2f tilespace = map.getTileSpacePosition(p);
	CBlob@ b = map.getBlobAtPosition(p);
	if (blob.getName() == "fakewire" && b !is null)
	{
...
bool isBuildableAtPos(CBlob@ this, Vec2f p, TileType buildTile, CBlob @blob, bool &out sameTile)
{
	...
	// tilemap check
	const bool buildSolid = (map.isTileSolid(buildTile) || buildTile >= 400 || (blob !is null && blob.isCollidable()));
	Vec2f tilespace = map.getTileSpacePosition(p);
	...
	if (buildTile > 0 && buildTile < 999 && blob is null && buildTile == map.getTile(offset).type)
	{
		sameTile = true;
		return false;
	}
	...
	if ((buildTile == CMap::tile_wood && backtile.type >= CMap::tile_wood_d1 && backtile.type <= CMap::tile_wood_d0) ||
			(buildTile == CMap::tile_castle && backtile.type >= CMap::tile_castle_d1 && backtile.type <= CMap::tile_castle_d0 ||
				(buildTile == CMap::tile_goldenbrick && backtile.type >= CMap::tile_goldenbrick_d1 && backtile.type <= CMap::tile_goldenbrick_d5)))
	{
		//repair like tiles
	}
